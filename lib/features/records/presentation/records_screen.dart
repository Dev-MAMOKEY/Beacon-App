...
class _RecordsScreenState extends ConsumerState<RecordsScreen> {
  ...
  Map<int, List<AttendanceRecordItem>> get _recordsByDay {
    ...
    for (final record in _records?.records ?? const <AttendanceRecordItem>[]) {
      ...
      //
      // **아직 확인되지 않은 백엔드 계약에 기대고 있다**: 서버가 `records[]`를
      // **KST 기준**으로 묶어 준다는 가정이다. UTC 기준으로 묶는다면
      // `2026-08-31T22:00Z` 세션이 8월 응답에 담겨 KST 9월 1일로 변환돼 여기서
      // 버려지고, 9월 응답에도 없어서 **어느 달에서도 보이지 않는다** — 그런데
      // 서버의 late/absent/attendanceRate는 그 세션을 8월에 세므로 캘린더와
      // 요약 카드가 말없이 어긋난다. 조정자가 백엔드에 확인 중이며, 그때까지
      // 현재 동작은 `records_screen_test.dart`의 월 경계 테스트가 고정한다.
